...
const MenuItem = ({
  ...
  const onModifyMenu = useCallback((): void => {
  ...
  const onStackPlus = useCallback((): void => {
    setModifyWeights(menus.map((menu: MenuTypes) => {
  ...
  const onStackMinus = useCallback((): void => {
    setModifyWeights(menus.map((menu: MenuTypes) => {
        return menu.id === id && menu.weight > 0 ? { ...menu, weight: weight - 1 } : menu;      
    }));
  }, [menus, weight])
    ...
    <>
      <div className='MenuItem'>
        ...
        <div className='MenuItem-Icons'>
          ...
          {/* 메뉴 삭제 버튼 */}
          <MdClose className='MenuItem-Icons-Close' onClick={() => onDelete(id)} />
        </div>
      </div>
